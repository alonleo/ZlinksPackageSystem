...
const form = ref<Partial<Notification>>({
  title: '',
  content: '',
  module: '',
  isPinned: 0,
...
const handleReset = () => { searchForm.value = { title: '', module: '', status: '' }; handleSearch() }
...
const handleCreate = () => {
  ...
  form.value = { title: '', content: '', module: '', isPinned: 0, status: 'unread' }
}

const handleSelect = async (row: Notification) => {
    ...
    if (valid) {
      ...
      try {
        if (isNew.value) {
          ...
          await notificationApi.update(selectedId.value!, { ...form.value })
...
const handleDeleteCurrent = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除通知"${form.value.title}"吗？`, '提示', {
      confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning',
    })
    deleting.value = true
    await notificationApi.delete(selectedId.value)
...
const handleFileChange = async (event: Event) => {
  ...
  try {
    const res = await notificationApi.importFile(file)
...
const handleExportCommand = async (command: string) => {
  if (command === 'xlsx' || command === 'json') {
    ...
    try {
      ...
      const blob = await notificationApi.exportFile(command)
      downloadBlob(blob, `通知数据.${ext}`)
      ElMessage.success('导出成功')
    } catch { ElMessage.error('导出失败') }
    finally { exporting.value = false }
  } else if (command === 'template-xlsx') {
    try {
      const blob = await notificationApi.downloadTemplate('xlsx')
    ...
    <div class="search-card">
      ...
      <el-form :model="searchForm" inline style="margin-top: 12px">
        ...
        <el-form-item>
          ...
          <el-button type="primary" :icon="Plus" @click="handleCreate">新增通知</el-button>
    ...
    <div class="content-row">
      <div class="detail-panel">
        <el-card class="detail-card">
          <template #header>
            <div class="card-header">
              <span>{{ isNew ? '新增通知' : isEditing ? '编辑通知' : '通知详情' }}</span>
